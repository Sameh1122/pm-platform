...
async function canAssignTargetRole(userId, targetRoleId) {
  ...
  const link = await prisma.roleAssignable.findFirst({ where:{ managerRoleId: { in: myRoleIds }, targetRoleId } });
  return !!link;
}
...
router.get('/projects/:id/assign',
  ...
  async (req,res)=>{
    ...
    const [users, roles, assignments] = await Promise.all([
      ...
      prisma.assignment.findMany({ where:{ projectId }, include:{ user:true, role:true }, orderBy:{ id:'asc' } })
    ]);
...
router.post('/projects/:id/assign',
  ...
  async (req,res)=>{
    ...
    if (!(await canAssignTargetRole(req.user.id, roleId))) return res.status(403).send('Forbidden: your manager role cannot assign this target role');
    try { await prisma.assignment.create({ data:{ projectId, userId, roleId } }); } catch {}
